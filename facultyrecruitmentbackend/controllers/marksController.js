...
const calculateWeights = ({ education, phd }) => {
    ...
    console.log(education.tenth_medium.toLowerCase(),education.twelfth_medium.toLowerCase())
    if (education.tenth_medium.toLowerCase() === 'english' && education.twelfth_medium.toLowerCase() === 'english') {
        med = 5;
    } else if (education.tenth_medium.toLowerCase() === 'tamil' && education.twelfth_medium.toLowerCase() === 'tamil') {
        med = 2;
    } else {
        med = 3.5;
    }
    ...
    // ✅ UG Degree Weight Calculation
// ✅ UG Degree Weight Calculation
 const normalizedDegree = education.ug_degree.replace(/[\s.]/g, '').toUpperCase();
if (normalizedDegree === 'BE' || normalizedDegree === 'BTECH') {
...
} else if (normalizedDegree === 'BSC' || normalizedDegree === 'BA') { // Adjusted condition to handle B.Sc. and B.A.
    if (education.ug_cgpa_percentage > 90) {
        udegmw8 = 20;
    } else if (education.ug_cgpa_percentage >= 81) {
        udegmw8 = 15;
    ...
    //console.log(education.pg_degree.replace(/[\s.]/g, '').toUpperCase(), education.mphil_year);
   const pgNormalized = education.pg_degree.replace(/[\s.]/g, '').toUpperCase();
    if (['MSC', 'MA', 'MCOM'].includes(pgNormalized) && education.mphil_year) {
        mphilwet = 5;
    }   
