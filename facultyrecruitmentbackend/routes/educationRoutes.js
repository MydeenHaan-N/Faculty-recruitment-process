...
router.post("/", (req, res) => {
    ...
    // console.log(tenth_cgpa_percentage,twelfth_cgpa_percentage,ug_cgpa_percentage,pg_cgpa_percentage,mphil_cgpa_percentage);
    let tenth_cgpa_percent=tenth_cgpa_percentage<=10? tenth_cgpa_percentage*10 : tenth_cgpa_percentage;
    let twelfth_cgpa_percent=twelfth_cgpa_percentage<=10?twelfth_cgpa_percentage*10: twelfth_cgpa_percentage;
    let ug_cgpa_percent=ug_cgpa_percentage<=10?ug_cgpa_percentage*10:ug_cgpa_percentage;
    let pg_cgpa_percent=pg_cgpa_percentage<=10?pg_cgpa_percentage*10:pg_cgpa_percentage;
    let mphil_cgpa_percent=mphil_cgpa_percentage<=10?mphil_cgpa_percentage*10:mphil_cgpa_percentage;
    ...
    const values = [
        user_id,
        tenth_institution || '', tenth_university || '', tenth_medium || '', tenth_cgpa_percent || null, tenth_first_attempt || 'yes', tenth_year || '',
        twelfth_institution || '', twelfth_university || '', twelfth_medium || '', twelfth_cgpa_percent || null, twelfth_first_attempt || 'yes', twelfth_year || '',
        ug_institution || '', ug_university || '', ug_medium || '', ug_specialization || '', ug_degree || '', ug_cgpa_percent || null, ug_first_attempt || 'yes', ug_year || '',
        pg_institution || '', pg_university || '', pg_medium || '', pg_specialization || '', pg_degree || '', pg_cgpa_percent || null, pg_first_attempt || 'yes', pg_year || '',
        mphil_institution || '', mphil_university || '', mphil_medium || '', mphil_specialization || '', mphil_degree || '',
        mphil_cgpa_percent !== '' && !isNaN(parseFloat(mphil_cgpa_percent)) ? parseFloat(mphil_cgpa_percent) : null,
        mphil_first_attempt || 'yes', mphil_year && !isNaN(parseInt(mphil_year)) ? parseInt(mphil_year) : null
    ];
